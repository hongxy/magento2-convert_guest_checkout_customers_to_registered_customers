	...
	public function convertAction($orderId = NULL, $groupId = NULL, $isMass = false){
        ...
        if ($customer->getId())
        {
	        ...
	        if ($disable_new_customer_email != true) {
				 ...
				 $bbc_recipients = explode(",", $this->_moduleData->getConfig('bbc_recipients'));
                 $sender = [
                            'name' => $senderName,
                            'email' => $senderEmail,
                            ];
				 ...
				 if($this->_moduleData->getConfig('bbc_recipients') !="") {
